...
const methodsFile = path.join(
  srcRoot,
  "gateway/server-methods-list.ts"
);
...
if (!methodsMatch) {
  console.error("[sync] Could not parse BASE_METHODS from server-methods-list.ts");
  process.exit(1);
}

const methods = Array.from(
  methodsMatch[1].matchAll(/"([^"]+)"/g),
  (m) => m[1]
);
