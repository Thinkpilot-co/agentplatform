...
export type BuildStep = "idle" | "sync" | "build" | "deploy" | "healthcheck" | "done" | "error";
...
export async function startRebuild(opts: RebuildOptions = {}): Promise<BuildState> {
  if (currentBuild.running) {
  ...
  try {
    ...
    if (doSync) {
      ...
      const syncCode = await runCommand(
        "npx",
        ["tsx", "scripts/sync-openclaw.ts", openclawSrc]
      );
      if (syncCode !== 0) {
    ...
    const runCode = await runCommand("docker", [
      ...
      "-v", `${process.cwd()}/instances.json:/app/instances.json`,
    ...
    currentBuild.running = false;
    currentBuild.finishedAt = Date.now();
    const elapsed = ((currentBuild.finishedAt - currentBuild.startedAt!) / 1000).toFixed(1);
